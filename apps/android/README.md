## Ozzobot Node (Android) (internal)

Modern Android node app: connects to the **Gateway WebSocket** (`_ozzobot-gw._tcp`) and exposes **Canvas + Chat + Camera**.
...
```bash
pnpm ozzobot gateway --port 18789 --verbose
```
...
```bash
ozzobot nodes pending
ozzobot nodes approve <requestId>
```
