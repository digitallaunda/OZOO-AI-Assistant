...
struct IOSGatewayChatTransport: OzzoChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OzzoChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OzzoChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OzzoChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OzzoChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OzzoChatAttachmentPayload]) async throws -> OzzoChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OzzoChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(OzzoChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(OzzoGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OzzoGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OzzoChatEventPayload.self)
                        {
