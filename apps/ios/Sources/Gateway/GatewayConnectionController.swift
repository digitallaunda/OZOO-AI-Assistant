...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [OzzoCapability.canvas.rawValue, OzzoCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(OzzoCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OzzoCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OzzoLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OzzoCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OzzoSystemCommand.notify.rawValue,
            OzzoSystemCommand.which.rawValue,
            OzzoSystemCommand.run.rawValue,
            OzzoSystemCommand.execApprovalsGet.rawValue,
            OzzoSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(OzzoCapability.camera.rawValue) {
            commands.append(OzzoCameraCommand.list.rawValue)
            commands.append(OzzoCameraCommand.snap.rawValue)
            commands.append(OzzoCameraCommand.clip.rawValue)
        }
        if caps.contains(OzzoCapability.location.rawValue) {
            commands.append(OzzoLocationCommand.get.rawValue)
        }
