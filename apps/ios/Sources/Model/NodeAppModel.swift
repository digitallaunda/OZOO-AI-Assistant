...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = OzzoCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = OzzoCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = OzzoCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = OzzoCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = OzzoCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__ozzo__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: OzzoLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for ozzo:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case OzzoLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case OzzoCanvasCommand.present.rawValue,
                 OzzoCanvasCommand.hide.rawValue,
                 OzzoCanvasCommand.navigate.rawValue,
                 OzzoCanvasCommand.evalJS.rawValue,
                 OzzoCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case OzzoCanvasA2UICommand.reset.rawValue,
                 OzzoCanvasA2UICommand.push.rawValue,
                 OzzoCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case OzzoCameraCommand.list.rawValue,
                 OzzoCameraCommand.snap.rawValue,
                 OzzoCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case OzzoScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: OzzoNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OzzoNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(OzzoLocationGetParams.self, from: req.paramsJSON)) ??
            OzzoLocationGetParams()
        ...
        let payload = OzzoLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OzzoCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(OzzoCanvasPresentParams.self, from: req.paramsJSON)) ??
                OzzoCanvasPresentParams()
        ...
        case OzzoCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OzzoCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(OzzoCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OzzoCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(OzzoCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OzzoCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(OzzoCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case OzzoCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.ozzobotA2UI) return JSON.stringify({ ok: false, error: "missing ozzoA2UI" });
              return JSON.stringify(globalThis.ozzobotA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OzzoCanvasA2UICommand.push.rawValue, OzzoCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == OzzoCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(OzzoCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try OzzoCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(OzzoCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try OzzoCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.ozzobotA2UI) return JSON.stringify({ ok: false, error: "missing ozzoA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.ozzobotA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case OzzoCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OzzoCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(OzzoCameraSnapParams.self, from: req.paramsJSON)) ??
                OzzoCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OzzoCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(OzzoCameraClipParams.self, from: req.paramsJSON)) ??
                OzzoCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(OzzoScreenRecordParams.self, from: req.paramsJSON)) ??
            OzzoScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> OzzoLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return OzzoLocationMode(rawValue: raw) ?? .off
    }
