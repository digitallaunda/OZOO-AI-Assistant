...
enum CommandResolver {
    private static let projectRootDefaultsKey = "ozzo.gatewayProjectRootPath"
    private static let helperName = "ozzo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/ozzo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/ozzo")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let ozzoPaths = self.ozzoManagedPaths(home: home)
        if !ozzoPaths.isEmpty {
            extras.insert(contentsOf: ozzoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + ozzoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func ozzoManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".ozzobot")
    ...
    static func ozzoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOzzoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/ozzo.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyOzzoInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.ozzoExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let ozzoPath = self.projectOzzoExecutable(projectRoot: root) {
                return [ozzoPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "ozzo", subcommand] + extraArgs
            }
            if let ozzoPath = self.ozzoExecutable(searchPaths: searchPaths) {
                return [ozzoPath, subcommand] + extraArgs
            }

            let missingEntry = """
            ozzo entrypoint missing (looked for dist/index.js or bin/ozzo.js); run pnpm build.
            """
    ...
    // Existing callers still refer to ozzoCommand; keep it as node alias.
    static func ozzoCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real ozzo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/ozzo"
        ...
        \(cliSection)
        \(projectSection)
        if command -v ozzo >/dev/null 2>&1; then
          CLI="$(command -v ozzo)"
          ozzo \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/ozzo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/ozzo.js"
            node "$PRJ/bin/ozzo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent ozzo"
          pnpm --silent ozzo \(quotedArgs);
        else
          echo "ozzo CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OzzoConfigFile.loadDict()
