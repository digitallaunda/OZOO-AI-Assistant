...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OzzoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "bot.ozzo", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OzzoKit.AnyCodable]()) {
                $0[$1.key] = OzzoKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or OZZOBOT_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: OzzoProtocol.AnyCodable?) -> [String: OzzoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OzzoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OzzoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OzzoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("ozzo.control.heartbeat")
    static let controlAgentEvent = Notification.Name("ozzo.control.agent")
}
