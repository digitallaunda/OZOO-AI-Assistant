...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OZZOBOT_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/ozzo")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("ozzo-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("ozzo-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/ozzo/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("ozzo") && $0.pathExtension == "log" }
            .max { lhs, rhs in
