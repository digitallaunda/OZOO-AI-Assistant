...
enum OzzoLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = OzzoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OzzoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OzzoLogging.bootstrapIfNeeded()
        let label = OzzoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OzzoOSLogHandler: LogHandler {
...
struct OzzoFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OzzoLogging.parseLabel(self.label)
