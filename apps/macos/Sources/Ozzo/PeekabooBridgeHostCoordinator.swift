...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "bot.ozzo", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OzzoPeekabooBridgeServices?
    ...
    private func startIfNeeded() async {
        ...
        let services = OzzoPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: PeekabooBridgeConstants.ozzobotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(PeekabooBridgeConstants.ozzobotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OzzoPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "bot.ozzo.peekaboo")
