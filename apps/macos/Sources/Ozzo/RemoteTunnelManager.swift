...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "bot.ozzo", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Ozzo run already has an SSH listener on the expected port (common after restarts),
