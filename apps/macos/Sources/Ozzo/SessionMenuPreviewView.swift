...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "bot.ozzo", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OzzoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OzzoSessionPreviewEntry,
    ...
    private static func cache(payload: OzzoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OzzoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OzzoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OzzoKit.AnyCodable]) -> [OzzoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OzzoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OzzoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OzzoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OzzoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OzzoChatMessage) -> String? {
