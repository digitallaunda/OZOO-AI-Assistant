...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.ozzobot/ozzo.json. Prefer OZZOBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.ozzobot/ozzo.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.ozzobot/ozzo.json. Restart the gateway to apply."
        }
