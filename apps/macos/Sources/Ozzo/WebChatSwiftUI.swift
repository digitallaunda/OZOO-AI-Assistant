...
private let webChatSwiftLogger = Logger(subsystem: "bot.ozzo", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OzzoChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OzzoChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OzzoChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OzzoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OzzoGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OzzoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OzzoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OzzoChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OzzoChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OzzoChatView>) -> NSViewController
    {
