...
struct OzzoMacCLI {
    static func main() async {
        ...
        default:
            fputs("ozzo-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      ozzo-mac discover [--timeout <ms>] [--json] [--include-local]
      ozzo-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      ozzo-mac connect
      ozzo-mac connect --url ws://127.0.0.1:18789 --json
      ozzo-mac discover --timeout 3000 --json
      ozzo-mac wizard --mode local
    """)
}
