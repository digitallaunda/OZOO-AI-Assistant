...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OzzoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OzzoProtocol.AnyCodable] = [
            "presence": .init([OzzoProtocol.AnyCodable(entry)]),
        ]
        let payload = OzzoProtocol.AnyCodable(payloadMap)
