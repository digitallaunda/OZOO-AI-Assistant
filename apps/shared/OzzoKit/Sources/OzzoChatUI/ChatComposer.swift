...
@MainActor
struct OzzoChatComposer: View {
    @Bindable var viewModel: OzzoChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OzzoChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OzzoChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OzzoPendingAttachment.id)
                { (att: OzzoPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OzzoPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OzzoChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OzzoChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OzzoChatTheme.subtleCard)
        .clipShape(Capsule())
    }
