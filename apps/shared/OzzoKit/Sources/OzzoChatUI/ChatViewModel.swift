...
private let chatUILogger = Logger(subsystem: "bot.ozzo", category: "OzzoChatUI")

@MainActor
@Observable
public final class OzzoChatViewModel {
    public private(set) var messages: [OzzoChatMessage] = []
    ...
    public var attachments: [OzzoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OzzoChatPendingToolCall] = []
    public private(set) var sessions: [OzzoChatSessionEntry] = []
    private let transport: any OzzoChatTransport
    ...
    private var pendingToolCallsById: [String: OzzoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OzzoChatTransport) {
    ...
    public var sessionChoices: [OzzoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [OzzoChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [OzzoChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OzzoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OzzoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OzzoChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [OzzoChatMessage]) -> [OzzoChatMessage] {
        var result: [OzzoChatMessage] = []
    ...
    private static func dedupeKey(for message: OzzoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OzzoChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> OzzoChatAttachmentPayload in
            OzzoChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OzzoChatSessionEntry {
        OzzoChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OzzoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OzzoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OzzoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OzzoChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OzzoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
