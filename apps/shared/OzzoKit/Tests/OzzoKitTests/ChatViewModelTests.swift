...
private final class TestChatTransport: @unchecked Sendable, OzzoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OzzoChatHistoryPayload]
    private let sessionsResponses: [OzzoChatSessionsListResponse]

    private let stream: AsyncStream<OzzoChatTransportEvent>
    private let continuation: AsyncStream<OzzoChatTransportEvent>.Continuation

    init(
        historyResponses: [OzzoChatHistoryPayload],
        sessionsResponses: [OzzoChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OzzoChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OzzoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OzzoChatHistoryPayload(
        ...
        attachments _: [OzzoChatAttachmentPayload]) async throws -> OzzoChatSendResponse
    {
        ...
        return OzzoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OzzoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OzzoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OzzoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OzzoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OzzoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OzzoChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OzzoChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OzzoChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OzzoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OzzoChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
