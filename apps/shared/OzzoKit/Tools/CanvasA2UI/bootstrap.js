...
import "@a2ui/lit/ui";
import { themeContext } from "@ozoo/a2ui-theme-context";
...
class OzooA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--ozoo-a2ui-inset-top, 0px)
        var(--ozoo-a2ui-inset-right, 0px)
        var(--ozoo-a2ui-inset-bottom, 0px)
        var(--ozoo-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--ozoo-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--ozoo-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--ozoo-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--ozoo-a2ui-empty-top, var(--ozoo-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.ozooA2UI = api;
    globalThis.ozoobotA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("ozoo:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("ozoo:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__ozooLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.ozooCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.ozoobotCanvasA2UIAction ??
      globalThis.ozooCanvasA2UIAction ??
      globalThis.ozoobotCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.ozooCanvasA2UIAction ||
          handler === globalThis.ozoobotCanvasA2UIAction
        ) {
...
customElements.define("ozoo-a2ui-host", OzooA2UIHost);
