...
This document describes how the Ozzo ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`ozzo acp` exposes an ACP agent over stdio and forwards prompts to a running
Ozzo Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Ozzo Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `ozzo acp` over stdio.

Example config:

```bash
ozzo config set gateway.remote.url wss://gateway-host:18789
ozzo config set gateway.remote.token
```

Example run:

```bash
ozzo acp --url wss://gateway-host:18789 --token
```
...
```bash
ozzo acp --session agent:main:main
ozzo acp --session agent:design:main
ozzo acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select "Ozzo ACP" to start a thread.

## Execution Model

- ACP client spawns `ozzo acp` and speaks ACP messages over stdio.
...
```bash
ozzo acp --session agent:main:main
ozzo acp --session-label "support inbox"
ozzo acp --reset-session
```
...
`ozzo acp` resolves the Gateway URL and auth from CLI flags or config:
