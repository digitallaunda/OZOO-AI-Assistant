...
- Jobs persist under `~/.ozzobot/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.ozzobot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.ozzobot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
ozzo cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
ozzo cron edit <jobId> --agent ops
ozzo cron edit <jobId> --clear-agent
```
```
...
```bash
ozzo system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`ozzo system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `OZOOBOT_SKIP_CRON`.
