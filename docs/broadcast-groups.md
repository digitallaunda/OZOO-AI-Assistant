...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Ozzo would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Ozzo would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/ozzo-alfred/
...
Workspace: /Users/pascal/ozzo-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.ozzobot/logs/gateway.log | grep broadcast
```
