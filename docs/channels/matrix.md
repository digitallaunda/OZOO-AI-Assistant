...
Matrix is an open, decentralized messaging protocol. Ozzo connects as a Matrix **user**
...
```bash
ozzo plugins install @ozzo/matrix
```

Local checkout (when running from a git repo):

```bash
ozzo plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `ozzo plugins install @ozzo/matrix`
   - From a local checkout: `ozzo plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Ozzo calls the same
     login endpoint, stores the access token in `~/.ozzobot/credentials/matrix/credentials.json`,
...
- On first connection, Ozzo requests device verification from your other sessions.
...
`~/.ozzobot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `ozzo pairing list matrix`
  - `ozzo pairing approve matrix <CODE>`
...
- On startup, Ozzo resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
