...
```bash
ozzo plugins install @ozzo/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
ozzo plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Ozzo" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `ozzo pairing list nextcloud-talk`
  - `ozzo pairing approve nextcloud-talk <CODE>`
