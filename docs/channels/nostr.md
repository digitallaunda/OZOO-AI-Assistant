...
read_when:
  - You want Ozzo to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Ozzo to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`ozzo onboard`) and `ozzo channels add` list optional channel plugins.
...
```bash
ozzo plugins install @ozzo/nostr
```

Use a local checkout (dev workflows):

```bash
ozzo plugins install --link <path-to-ozzo>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "ozzo@example.com",
        "lud16": "ozzo@example.com"
      }
    }
  }
}
