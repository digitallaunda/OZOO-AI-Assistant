...
Tlon is a decentralized messenger built on Urbit. Ozzo connects to your Urbit ship and can
...
```bash
ozzo plugins install @ozzo/tlon
```

Local checkout (when running from a git repo):

```bash
ozzo plugins install ./extensions/tlon
```
...
Use these with `ozzo message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Ozzo replies in-thread.
