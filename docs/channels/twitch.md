...
read_when:
  - Setting up Twitch chat integration for Ozzo
---
# Twitch (plugin)

Twitch chat support via IRC connection. Ozzo connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ozzo plugins install @ozzo/twitch
```

Local checkout (when running from a git repo):

```bash
ozzo plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `OZOOBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "ozzo",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use OZOOBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
OZOOBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
