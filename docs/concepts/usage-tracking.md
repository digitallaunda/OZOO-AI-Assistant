...
- `/usage cost` in chats: local cost summary aggregated from Ozzo session logs.
- CLI: `ozzo status --usage` prints a full per-provider breakdown.
- CLI: `ozzo channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
