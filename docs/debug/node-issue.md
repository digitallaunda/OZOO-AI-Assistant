...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Ozzo
---

# Node + tsx "__name is not a function" crash

## Summary
Running Ozzo via Node with `tsx` fails at startup with:

```
[ozzo] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `ozzo status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch ozzo.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node ozzo.mjs status` works on Node 25.
