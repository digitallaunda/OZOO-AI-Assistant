---
summary: "Research notes: offline memory system for Ozoo workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/ozzo) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Ozzo integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Ozoo-style workspace (`agents.defaults.workspace`, default `~/ozzo`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/ozzo` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Ozoo to see every session.
...
Recommendation: **deep integration in Ozzo**, but keep a separable core library.

### Why integrate into Ozzo?
- Ozzo already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `ozzo memory recall "…" --k 25 --since 30d`
  - `ozzo memory reflect --since 7d`
