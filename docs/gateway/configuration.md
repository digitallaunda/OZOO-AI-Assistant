---
summary: "All configuration options for ~/.ozzobot/ozzo.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Ozzo reads an optional **JSON5** config from `~/.ozzobot/ozzo.json` (comments + trailing commas allowed).

If the file is missing, Ozzo uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/ozzo`). You usually only need a config to:
...
Ozzo only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `ozzo doctor`, `ozzo logs`, `ozzo health`, `ozzo status`, `ozzo service`, `ozzo help`).
- Run `ozzo doctor` to see the exact issues.
- Run `ozzo doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `ozzo config set`. Keep a backup of `~/.ozzobot/ozzo.json`.
...
```bash
ozzo gateway call config.get --params '{}' # capture payload.hash
ozzo gateway call config.apply --params '{
...
```bash
ozzo gateway call config.get --params '{}' # capture payload.hash
ozzo gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/ozzo" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@ozzo", "reisponde"] }
      }
    ]
  },
  channels: {
...
```json5
// ~/.ozzobot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [
    { id: "main", workspace: "~/ozzo" }
  ]
}
```
...
{ "$include": "/etc/ozzo/base.json5" } // absolute
...
```json5
// ~/.ozzobot/clients/mueller/agents.json5
[
...
```json5
// ~/.ozzobot/clients/mueller/broadcast.json5
{
...
Ozzo reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Ozzo runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `OZOOBOT_LOAD_SHELL_ENV=1`
- `OZOOBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
Ozzo stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.ozzobot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.ozzobot/credentials/oauth.json` (or `$OZOOBOT_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.ozzobot/agent/*` (migrated by `ozzo doctor` into `~/.ozzobot/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `OZOOBOT_OAUTH_DIR`
- Agent dir (default agent root override): `OZOOBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Ozzo imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Ozzo derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/ozzo/ozzo-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/ozzo/ozzo.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.ozzobot/credentials/whatsapp/biz
          // authDir: "~/.ozzobot/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `ozzo doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@ozzo", "ozzo", "ozzo"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@ozzo"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/ozzo-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.ozzobot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/ozzo-home" },
      { id: "work", workspace: "~/ozzo-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `ozzo.json`).
...
Ozzo starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["ozzo-dm"]          // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {               // guild id (preferred) or slug
          slug: "friends-of-ozzo",
...
Ozzo starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `ozzo plugins install @ozzo/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Ozzo starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Ozzo spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Ozzo
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/ozzo" } }
}
```
...
When a file exceeds this limit, Ozzo logs a warning and injects a truncated
...
message envelopes). If unset, Ozzo uses the host timezone at runtime.
...
`[ozzo]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[ozzo]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[ozzo]"`, Ozzo will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Ozzo generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.ozzobot/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Ozoo"`).
...
Ozzo also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Ozzo currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `ozzo configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:ozzo`: all built-in Ozzo tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.ozzobot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.ozzobot/sandboxes",
        docker: {
        ...
        browser: {
          enabled: false,
          image: "ozzo-sandbox-browser:bookworm-slim",
          containerPrefix: "ozzo-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Ozzo uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.ozzobot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Ozzo config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Ozzo writes/merges a `models.json` into
`~/.ozzobot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Ozzo uses
...
- Shortcut: `ozzo onboard --auth-choice opencode-zen`.
...
Shortcut: `ozzo onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `ozzo onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `ozzo onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `ozzo onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `ozzo onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OZOOBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.ozzobot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.ozzobot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.ozzobot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Ozzo stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.ozzobot/skills` (workspace skills
...
from `~/.ozzobot/extensions`, `<workspace>/.ozzobot/extensions`, plus any
...
### `browser` (ozzo-managed browser)

Ozzo can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for ozzo and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Ozzo.app menubar, or `ozzo gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      ozzo: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/ozzo" }
...
- When a connection comes from one of these IPs, Ozzo uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `ozzo gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OZOOBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OZOOBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Ozzo
...
macOS app behavior:
- Ozzo.app watches `~/.ozzobot/ozzo.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.ozzobot/ozzo.json` (or `OZOOBOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.ozzobot/ozzo.json` (or `OZOOBOT_CONFIG_PATH`)
...
- `OZOOBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `ozzo --dev …` → uses `~/.ozzobot-dev` + shifts ports from base `19001`
- `ozzo --profile <name> …` → uses `~/.ozzobot-<name>` (port via config/env/flags)
...
Example:
```bash
OZOOBOT_CONFIG_PATH=~/.ozzobot/a.json \
OZOOBOT_STATE_DIR=~/.ozzobot-a \
ozzo gateway --port 19001
```
...
Gmail helper config (used by `ozzo webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "ozzo@gmail.com",
...
- Set `OZOOBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Ozzo defaults `serve.path` to `/` so
...
Default root: `~/ozzo/canvas`  
Default port: `18793` (chosen to avoid the ozzo browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__ozzo/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__ozzo__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `OZOOBOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.ozzobot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.ozzobot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.ozzobot/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_ozzo-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_ozzo-bridge._tcp` under `~/.ozzobot/dns/` using the standard discovery domain `ozzo.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `ozzo.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `ozzo.internal` via that server
