...
- During heartbeat runs, Ozzo treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `ozzo sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Ozzo skips the heartbeat run entirely (no model call).
...
```bash
ozzo system event --text "Check for urgent follow-ups" --mode now
```
