...
- Replaces the legacy `gateway` command. CLI entry point: `ozzo gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
ozzo gateway --port 18789
# for full debug/trace logs in stdio:
ozzo gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
ozzo gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.ozzobot/ozzo.json` (or `OZOOBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__ozzo__/canvas/` from `~/ozzo/canvas`. Disable with `canvasHost.enabled=false` or `OZOOBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OZOOBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OZOOBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.ozoo.<profile>` (legacy `com.ozzobot.*` may still exist)
- Linux: `ozzo-gateway-<profile>.service`
- Windows: `Ozzo Gateway (<profile>)`

Install metadata is embedded in the service config:
- `OZOOBOT_SERVICE_MARKER=ozzo`
- `OZOOBOT_SERVICE_KIND=gateway`
- `OZOOBOT_SERVICE_VERSION=<version>`
...
```bash
ozzo --dev setup
ozzo --dev gateway --allow-unconfigured
...
- `OZOOBOT_STATE_DIR=~/.ozzobot-dev`
- `OZOOBOT_CONFIG_PATH=~/.ozzobot-dev/ozzo.json`
- `OZOOBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/ozzo-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `OZOOBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OZOOBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `OZOOBOT_CONFIG_PATH`
- unique `OZOOBOT_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
ozzo --profile main gateway install
ozzo --profile rescue gateway install
```

Example:
```bash
OZOOBOT_CONFIG_PATH=~/.ozzobot/a.json OZOOBOT_STATE_DIR=~/.ozzobot-a ozzo gateway --port 19001
OZOOBOT_CONFIG_PATH=~/.ozzobot/b.json OZOOBOT_STATE_DIR=~/.ozzobot-b ozzo gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `ozzo health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `ozzo`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `ozzo gateway install` writes `~/Library/LaunchAgents/bot.ozoo.gateway.plist`
    (or `bot.ozoo.<profile>.plist`; legacy `com.ozzobot.*` is cleaned up).
  - `ozzo doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Ozzo profile services.
  ...
  - Cleanup: `ozzo gateway uninstall` (current service) and `ozzo doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `ozzo gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Ozzo.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.ozoo.gateway` (or `bot.ozoo.<profile>`; legacy `com.ozzobot.*` labels still unload cleanly).
- To stop it cleanly, use `ozzo gateway stop` (or `launchctl bootout gui/$UID/bot.ozoo.gateway`).
- To restart, use `ozzo gateway restart` (or `launchctl kickstart -k gui/$UID/bot.ozoo.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `ozzo gateway install` first.
  - Replace the label with `bot.ozoo.<profile>` when running a named profile.

## Supervision (systemd user unit)
Ozzo installs a **systemd user service** by default on Linux/WSL2. We
...
`ozzo gateway install` writes the user unit. `ozzo doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/ozzo-gateway[-<profile>].service`:
```
[Unit]
Description=Ozzo Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/ozzo gateway --port 18789
Restart=always
RestartSec=5
Environment=OZOOBOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now ozzo-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/ozzo-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now ozzo-gateway[-<profile>].service
```
...
## CLI helpers
- `ozzo gateway health|status` — request health/status over the Gateway WS.
- `ozzo message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `ozzo agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `ozzo gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `ozzo gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `ozzo gateway` and the legacy TCP control port.
