---
summary: "Quick troubleshooting guide for common Ozzo failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Ozzo misbehaves, here's how to fix it.
...
|---|---|---|
| `ozzo status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `ozzo status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `ozzo status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `ozzo gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `ozzo channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `ozzo gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `ozzo logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `ozzo status --all` (it redacts tokens). If you paste `ozzo status`, consider setting `OZOOBOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  ozzo models auth setup-token --provider anthropic
  ```
...
```bash
ozzo models auth paste-token --provider anthropic
ozzo models status
```
...
**Logs:**
- Preferred: `ozzo logs --follow`
- File logs (always): `/tmp/ozzo/ozzo-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$OZOOBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u ozzo-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  ozzo config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  ozzo config set gateway.mode remote
  ozzo config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `ozzo setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.ozzobot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `ozzo doctor`
...
- then run `ozzo sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `ozzo gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `ozzo gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `ozzo gateway status` and `ozzo doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `OZOOBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `ozzo gateway status` shows a config mismatch**
...
- Fix: rerun `ozzo gateway install --force` from the same `--profile` / `OZOOBOT_STATE_DIR` you want the service to use.

**If `ozzo gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `ozzo doctor` to update it (or `ozzo gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `OZOOBOT_GATEWAY_TOKEN`) and restart the service.

**If `ozzo gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/ozzo` on disk.
...
Symptoms: `pwd` or file tools show `~/.ozzobot/sandboxes/...` even though you
...
Ozzo intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `ozzo models list` or
  `ozzo models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OZOOBOT_CONFIG_PATH:-$HOME/.ozzobot/ozzo.json}"
```
...
tail -f "$(ls -t /tmp/ozzo/ozzo-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
ozzo logs --follow | grep "pairing request"
```
...
- ❌ `@ozzo` + image
- ✅ `@ozzo check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.ozzobot/agents/<agentId>/sessions/
```
...
ozzo logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
ozzo gateway --verbose
```

If you’re logged out / unlinked:

```bash
ozzo channels logout
trash "${OZOOBOT_STATE_DIR:-$HOME/.ozzobot}/credentials" # if logout can't cleanly remove everything
ozzo channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/ozzo/ozzo-*.log | head -1)" | tail -20
```

### High Memory Usage

Ozzo keeps conversation history in memory.
...
Ozzo now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `ozzo doctor` reports every invalid entry.
- `ozzo doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `ozzo logs`, `ozzo health`, `ozzo status`, `ozzo gateway status`, and `ozzo gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/ozzo/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `ozzo models status` (CLI).
...
```bash
curl -fsSL https://ozzo.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `ozzo channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Ozzo scrubs/normalizes tool
...
```bash
tccutil reset All bot.ozoo.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/ozzo/ozzo/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
ozzo gateway status
ozzo gateway stop
# Or: launchctl bootout gui/$UID/bot.ozoo.gateway (replace with bot.ozoo.<profile>; legacy com.ozzobot.* still works)
```
...
Ensure the global `ozzo` CLI is installed and matches the app version:
```bash
ozzo --version
npm install -g ozzo@<version>
```
...
#   ${OZOOBOT_CONFIG_PATH:-$HOME/.ozzobot/ozzo.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
ozzo gateway --verbose
ozzo channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/ozzo/ozzo-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$OZOOBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.ozzobot/logs/...`; profiles use `~/.ozzobot-<profile>/logs/...`)<br />Linux: `journalctl --user -u ozzo-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Ozzo Gateway (<profile>)" /V /FO LIST` |
| Session files | `$OZOOBOT_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$OZOOBOT_STATE_DIR/media/` |
| Credentials | `$OZOOBOT_STATE_DIR/credentials/` |
...
# ozzo gateway uninstall

trash "${OZOOBOT_STATE_DIR:-$HOME/.ozzobot}"
ozzo channels login         # re-pair WhatsApp
ozzo gateway restart           # or: ozzo gateway
```
...
1. Check logs first: `/tmp/ozzo/` (default: `ozzo-YYYY-MM-DD.log`, or your configured `logging.file`)
