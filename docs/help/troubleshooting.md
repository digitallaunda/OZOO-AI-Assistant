...
You can also set `OZOOBOT_VERBOSE=1` instead of the flag.
...
### `docs.ozzo.bot` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.ozzo.bot` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.ozzo.bot` to the allowlist, then retry.
...
- Check the allowlist: `ozzo config get agents.defaults.models`
...
If you can, include the relevant log tail from `ozzo logs --follow`.
