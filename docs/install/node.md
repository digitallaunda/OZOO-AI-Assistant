...
read_when:
  - "You installed Ozzo but `ozzo` is “command not found”"
...
Ozzo’s runtime baseline is **Node 22+**.

If you can run `npm install -g ozzo@latest` but later see `ozzo: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
