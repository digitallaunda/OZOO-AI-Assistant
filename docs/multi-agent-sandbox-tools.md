...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `ozzo sandbox explain`.
...
Legacy `agent.*` configs are migrated by `ozzo doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=ozzo.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${OZOOBOT_STATE_DIR:-$HOME/.ozzobot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
