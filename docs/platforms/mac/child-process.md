...
service via the external `ozzo` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.ozoo.gateway`
  (or `bot.ozoo.<profile>` when using `--profile`/`OZOOBOT_PROFILE`; legacy `com.ozzobot.*` is supported).
