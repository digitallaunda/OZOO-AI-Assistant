---
summary: "Ozzo on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Ozzo on a Raspberry Pi
  - Running Ozzo on ARM devices
  - Building a cheap always-on personal AI
---

# Ozzo on Raspberry Pi

## Goal

Run a persistent, always-on Ozzo Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Ozzo features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled ozzo

# Enable if not
sudo systemctl enable ozzo

# Start on boot
sudo systemctl start ozzo
```
...
```bash
# Check logs
journalctl -u ozzo --no-pager -n 100

# Common fix: rebuild
cd ~/ozzo  # if using hackable install
npm run build
sudo systemctl restart ozzo
```
