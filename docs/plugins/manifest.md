...
read_when:
  - You are building a Ozzo plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (ozzo.plugin.json)

Every plugin **must** ship a `ozzo.plugin.json` file in the **plugin root**.
Ozzo uses this manifest to validate configuration **without executing plugin
