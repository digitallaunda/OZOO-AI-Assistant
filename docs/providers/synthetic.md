---
summary: "Use Synthetic's Anthropic-compatible API in Ozzo"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Ozzo registers it as the
...
```bash
ozzo onboard --auth-choice synthetic-api-key
```
...
Note: Ozzo's Anthropic client appends `/v1` to the base URL, so use
