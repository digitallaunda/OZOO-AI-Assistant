---
summary: "Use Venice AI privacy-focused models in Ozzo"
read_when:
  - You want privacy-focused inference in Ozzo
...
```bash
ozzo onboard --auth-choice venice-api-key
```
...
```bash
ozzo chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Ozzo shows all available Venice models. Pick based on your needs:
...
```bash
ozzo models set venice/claude-opus-45
ozzo models set venice/llama-3.3-70b
```

List all available models:

```bash
ozzo models list | grep venice
```

## Configure via `ozzo configure`

1. Run `ozzo configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Ozzo automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
ozzo chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
ozzo chat --model venice/claude-opus-45

# Use uncensored model
ozzo chat --model venice/venice-uncensored

# Use vision model with image
ozzo chat --model venice/qwen3-vl-235b-a22b

# Use coding model
ozzo chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `ozzo models list` to see currently available models. Some models may be temporarily offline.
