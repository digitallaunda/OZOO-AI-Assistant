...
Deploy Ozzo on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/ozzo-railway-template" target="_blank" rel="noreferrer">Deploy on Railway</a>
...
- `https://<your-railway-domain>/ozzo` — Control UI

## What you get

- Hosted Ozzo Gateway + Control UI
...
- `OZOOBOT_STATE_DIR=/data/.ozoobot` (recommended)
- `OZOOBOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `OZOOBOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Ozzo state + workspace so you can migrate to another host without losing config or memory.
