...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.ozzobot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.ozzobot/node.json` (node id + pairing token).
...
Path: `~/.ozzobot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.ozzobot/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.ozzobot/exec-approvals.sock` (0600).
