...
Delivery:
- Publish as `@ozzobot/plugin-sdk` (or export from core under `ozzobot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `OzzoPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OzzoConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OzzoConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OzzoConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: OzzoConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OzzoConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OzzoConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@ozzobot/plugin-sdk`.
- Add `api.runtime` to `OzzoPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `ozzoRuntime: ">=2026.2.0"`).
