...
Ozzo features that can generate provider usage or paid API calls.
...
- `ozzo status --usage` and `ozzo channels list` show provider **usage windows**
...
Ozzo can pick up credentials from:
...
### 8) Model scan / probe
`ozzo models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
