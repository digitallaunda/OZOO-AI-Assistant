...
Deploy Ozzo on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/ozzo/ozzo" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Ozzo:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OZOOBOT_STATE_DIR
        value: /data/.ozoobot
      - key: OZOOBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: OZOOBOT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/ozzo`.
...
If you use the original Ozzo repository, Render will not auto-deploy your Ozzo. To update it, run a manual Blueprint sync from the dashboard.
...
For Ozzo, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Ozzo host.
