---
summary: "First-run onboarding flow for Ozzo (macOS app)"
read_when:
...
- Writes credentials to `~/.ozzobot/credentials/oauth.json`
...
The app can install the global `ozzo` CLI via npm/pnpm so terminal
...
On the first agent run, Ozzo bootstraps a workspace (default `~/ozzo`):
...
```bash
ozzo webhooks gmail setup --account you@gmail.com
```
...
- `~/.ozzobot/credentials/oauth.json`
