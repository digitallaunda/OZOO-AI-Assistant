---
summary: "Direct `ozzo agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `ozzo agent` (direct agent runs)

`ozzo agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `ozzo message --target`).
...
```bash
ozzo agent --to +15555550123 --message "status update"
...
ozzo agent --to +15555550123 --message "Summon reply" --deliver
