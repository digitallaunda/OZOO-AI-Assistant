---
summary: "Chrome extension: let Ozzo drive your existing Chrome tab"
read_when:
...
The Ozzo Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate ozzo-managed Chrome profile.
...
Ozzo then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Ozzo release (npm package) as static files. There is no separate “build” step.

After upgrading Ozzo:
- Re-run `ozzo browser extension install` to refresh the installed files under your Ozzo state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Ozzo ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Ozzo to control.
...
- `ON`: attached; Ozzo can drive that tab.
...
Debugging: `ozzo sandbox explain`
...
`ozzo browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `ozzo browser extension install` first to copy the extension to a stable location under your Ozzo state directory.
...
- **This is not isolated** like the dedicated ozzo-managed profile.
