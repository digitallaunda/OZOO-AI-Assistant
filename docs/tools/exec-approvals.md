...
- **gateway host** → `ozzo` process on the gateway machine
...
`~/.ozzobot/exec-approvals.json` directly.

CLI: `ozzo approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
