---
summary: "Agent tool surface for Ozzo (browser, canvas, nodes, message, cron) replacing legacy `ozzo-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `ozzo-*` skills
---

# Tools (Ozzo)

Ozzo exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `ozzo-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `ozzo.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Ozzo logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`ozzo node run`).
...
Notes:
- Requires a Brave API key (recommended: `ozzo configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated ozzo browser.
...
- Quick smoke: `ozzo nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `ozzo gateway` restart in-place)
