...
export function listGoogleChatAccountIds(cfg: OzzoConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: OzzoConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: OzzoConfig): ResolvedGoogleChatAccount[] {
