...
`ozzo.invoke --each`) without adding new Ozzo code per workflow.
...
This extension depends on Ozzo internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Ozzo extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.ozzo/extensions` as a standalone plugin directory.
