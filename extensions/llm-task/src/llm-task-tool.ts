...
// NOTE: This extension is intended to be bundled with Ozzo.
// When running from source (tests/dev), Ozzo internals live under src/.
...
export function createLlmTaskTool(api: OzzoPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via ozzo.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-llm-task-"));
