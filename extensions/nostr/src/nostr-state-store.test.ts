...
import type { PluginRuntime } from "ozzo/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OZZO_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-nostr-"));
  process.env.OZZO_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OZZO_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".ozzo");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.OZZO_STATE_DIR;
    else process.env.OZZO_STATE_DIR = previous;
