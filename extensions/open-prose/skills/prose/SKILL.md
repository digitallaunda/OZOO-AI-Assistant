...
metadata: {"ozzo":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Ozzo `sessions_spawn`
- **File I/O** == Ozzo `read`/`write`
- **Remote fetch** == Ozzo `web_fetch` (or `exec` with curl when POST is required)
