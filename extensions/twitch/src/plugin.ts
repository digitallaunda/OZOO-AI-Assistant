/**
 * Twitch channel plugin for Ozzo.
 *
...
import { buildChannelConfigSchema } from "ozzo/plugin-sdk";
 ...
 * for Ozzo. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OzzoConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OzzoConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OzzoConfig): boolean => {
