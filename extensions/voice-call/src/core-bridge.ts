...
function resolveOzzoRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.OZZO_ROOT?.trim() || process.env.OZZO_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set OZZO_ROOT (or legacy OZZO_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveOzzoRoot();
