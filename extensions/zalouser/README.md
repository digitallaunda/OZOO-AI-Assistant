...
Ozzo extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
ozzo channels login --channel zalouser              # Login via QR
ozzo channels login --channel zalouser --account work
...
```bash
ozzo directory self --channel zalouser
ozzo directory peers list --channel zalouser --query "name"
ozzo directory groups list --channel zalouser --query "work"
ozzo directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
ozzo message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
ozzo message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `ozzo channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
ozzo channels login --channel zalouser --account work
ozzo message send --channel zalouser --account work --target <id> --message "Hello"
