...
#
# Suggested cron: */30 * * * * /home/admin/ozzo/scripts/auth-monitor.sh
#
# Environment variables:
# NOTIFY_PHONE - Phone number to send Ozzo notification (e.g., +1234567890)
# NOTIFY_NTFY - ntfy.sh topic for push notifications (e.g., ozzo-alerts)
...
STATE_FILE="$HOME/.ozzo/auth-monitor-state"
...
send_notification() {
	...
	# Send via Ozzo if phone configured and auth still valid
	if [ -n "$NOTIFY_PHONE" ]; then
		# Check if we can still use ozzo
		if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
			echo "Sending via Ozzo to $NOTIFY_PHONE..."
			ozzo send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
		fi
	fi
...
if [ "$DIFF_MS" -lt 0 ]; then
	send_notification "Claude Code auth EXPIRED! Ozzo is down. Run: ssh l36 '~/ozzo/scripts/mobile-reauth.sh'" "urgent"
	exit 1
