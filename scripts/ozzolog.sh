...
# Ozzo Logging Utility
# Simplifies access to Ozzo logs using macOS unified logging system
...
handle_sudo_error() {
	...
	echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
	echo -e "ozzolog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
	cat << EOF
ozzolog - Ozzo Logging Utility

USAGE:
	ozzolog [OPTIONS]

DESCRIPTION:
	View Ozzo logs with full details (bypasses Apple's privacy redaction).
	Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
	Ozzo logs flow through the macOS unified log (subsystem: bot.ozzo).
...
QUICK START:
	ozzolog -n 100					Show last 100 lines from all components
	ozzolog -f							Follow logs in real-time
	ozzolog -e							Show only errors
	ozzolog -c ServerManager	Show logs from ServerManager only
...
EXAMPLES:
	ozzolog									Show last 50 lines from past 5 minutes (default)
	ozzolog -f							Stream logs continuously
	ozzolog -n 100					Show last 100 lines
	ozzolog -e							Show only recent errors
	ozzolog -l 30m -n 200		Show last 200 lines from past 30 minutes
	ozzolog -c ServerManager	Show recent ServerManager logs
	ozzolog -s "fail"				Search for "fail" in recent logs
	ozzolog --server -e			Show recent server errors
	ozzolog -f -d						Stream debug logs continuously
...
list_categories() {
	echo -e "${BLUE}Fetching Ozzo log categories from the last hour...${NC}\n"
...
if [[ "$STREAM_MODE" == true ]]; then
	...
	echo -e "${GREEN}Streaming Ozzo logs continuously...${NC}"
