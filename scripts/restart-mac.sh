#!/usr/bin/env bash
# Reset Ozzo: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${OZOO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Ozzo.app/Contents/MacOS/Ozzo"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Ozzo"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Ozzo"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Ozzo"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.ozzo.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/ozzo-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${OZOO_RESTART_LOG:-/tmp/ozzo-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${OZOO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.ozzo/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
	case "${arg}" in
		...
		--help|-h)
			...
			log ""
			log "Env:"
			log "	OZOO_GATEWAY_WAIT_SECONDS=0		Wait time before gateway port check (unsigned only)"
			log ""
			log "Unsigned recovery:"
			log "	node ozzo.mjs daemon install --force --runtime node"
			log "	node ozzo.mjs daemon restart"
			log ""
			log "Reset unsigned overrides:"
			log "	rm ~/.ozzo/disable-launchagent"
			log ""
...
stop_launch_agent() {
	launchctl bootout gui/"$UID"/bot.ozzo.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Ozzo instances"
kill_all_ozzo
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Ozzo"
...
choose_app_bundle() {
	...
	if [[ -d "/Applications/Ozzo.app" ]]; then
		APP_BUNDLE="/Applications/Ozzo.app"
		return 0
	fi

	if [[ -d "${ROOT_DIR}/dist/Ozzo.app" ]]; then
		APP_BUNDLE="${ROOT_DIR}/dist/Ozzo.app"
		if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
			fail "dist/Ozzo.app missing Sparkle after packaging"
		fi
		return 0
	fi

	fail "App bundle not found. Set OZOO_APP_BUNDLE to your installed Ozzo.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
	run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node ozzo.mjs daemon install --force --runtime node"
	run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node ozzo.mjs daemon restart"
	...
	GATEWAY_PORT="$(
		...
		try {
			const raw = fs.readFileSync(path.join(process.env.HOME, ".ozzo", "ozzo.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
	log "OK: Ozzo is running."
else
	fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
	run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/bot.ozzo.gateway.plist' | head -n 40 || true"
fi
