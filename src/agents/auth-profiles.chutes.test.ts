...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.OZZO_AGENT_DIR;
    else process.env.OZZO_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-chutes-"));
    process.env.OZZO_STATE_DIR = tempDir;
    process.env.OZZO_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
