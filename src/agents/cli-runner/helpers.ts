...
/**
 * Cleanup suspended Ozzo CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OzzoConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-cli-images-"));
  const paths: string[] = [];
