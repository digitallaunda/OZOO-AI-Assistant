...
import { __testing, createOzzoCodingTools } from "./pi-tools.js";
...
describe("createOzzoCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOzzoCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "ozzo-outside.txt");
