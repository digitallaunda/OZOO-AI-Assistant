...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_OZZO_BROWSER_COLOR,
    ...
    defaultProfile: "ozzo",
    profiles: {
      ozzo: { cdpPort: params.cdpPort, color: DEFAULT_OZZO_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    args.push("-e", `OZZO_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `OZZO_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `OZZO_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `OZZO_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `OZZO_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, "ozzo") : null;
  const shouldReuse =
