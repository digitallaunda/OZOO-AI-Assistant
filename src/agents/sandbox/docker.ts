...
function validateDockerArg(value: string, argType: "container" | "image" | "generic"): void {
  ...
  // Additional length check for all arguments
  if (value.length > 1000) {
    throw new Error(`Invalid docker argument: too long (max 1000 characters)`);
  }
}
...
export function execDocker(args: string[], opts?: { allowFailure?: boolean }) {
  // Security: Validate all arguments before passing to spawn
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "ozzo.sandbox=1");
  args.push("--label", `ozzo.sessionKey=${params.scopeKey}`);
  args.push("--label", `ozzo.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `ozzo.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "ozzo.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`ozzo sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`ozzo sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("ozzo sandbox recreate --all");
}
