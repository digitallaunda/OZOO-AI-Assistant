...
export function resolveOzzoMetadata(
  frontmatter: ParsedSkillFrontmatter,
): OzzoSkillMetadata | undefined {
  const raw = getFrontmatterValue(frontmatter, "metadata");
  if (!raw) return undefined;
  try {
    const parsed = JSON5.parse(raw) as { ozzo?: unknown } & Partial<
      Record<typeof LEGACY_MANIFEST_KEY, unknown>
    >;
    if (!parsed || typeof parsed !== "object") return undefined;
    const metadataRaw = parsed.ozzo ?? parsed[LEGACY_MANIFEST_KEY];
