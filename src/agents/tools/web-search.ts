...
type WebSearchConfig = NonNullable<OzzoConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OzzoConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "perplexity") {
    return {
      ...
      docs: "https://docs.ozzo.bot/tools/web",
    };
  }
  return {
    error: "missing_brave_api_key",
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("ozzo configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.ozzo.bot/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://ozzo.bot",
      "X-Title": "Ozzo Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
