...
import { scheduleGatewaySigusr1Restart, triggerOzzoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting ozzo in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOzzoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting ozzo via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
