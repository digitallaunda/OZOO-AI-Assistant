...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-commands-"));
...
function buildParams(commandBody: string, cfg: OzzoConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
