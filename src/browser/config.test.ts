...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and OZZO blue color", () => {
    const resolved = resolveBrowserConfig(undefined);
    expect(resolved.enabled).toBe(true);
    expect(resolved.controlPort).toBe(18791);
    expect(resolved.color).toBe("#0066CC");
    ...
    const ozzo = resolveProfile(resolved, "ozzo");
    expect(ozzo?.driver).toBe("ozzo");
    expect(ozzo?.cdpPort).toBe(18800);
    expect(ozzo?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from OZZO_GATEWAY_PORT when unset", () => {
    const prev = process.env.OZZO_GATEWAY_PORT;
    process.env.OZZO_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const ozzo = resolveProfile(resolved, "ozzo");
      expect(ozzo?.cdpPort).toBe(19012);
      expect(ozzo?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.OZZO_GATEWAY_PORT;
      } else {
        process.env.OZZO_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const ozzo = resolveProfile(resolved, "ozzo");
      expect(ozzo?.cdpPort).toBe(19022);
      expect(ozzo?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.OZZO_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "ozzo");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        ozzo: { cdpPort: 18792, color: "#0066CC" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("ozzo");
  });
});
