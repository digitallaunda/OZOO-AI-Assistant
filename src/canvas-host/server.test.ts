...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("ozzoCanvasA2UIAction");
    expect(out).toContain("ozzoSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("ozzoSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.ozzoA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__ozzo__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
