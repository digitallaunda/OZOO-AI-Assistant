...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.ozzoCanvasA2UIAction ||
        window.webkit.messageHandlers.ozzoCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.ozzoCanvasA2UIAction &&
        typeof window.ozzoCanvasA2UIAction.postMessage === "function") ||
      (window.ozzoCanvasA2UIAction &&
        typeof window.ozzoCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.ozzoSendUserAction === "function" ? window.ozzoSendUserAction : undefined;
  if (!window.ozzoSendUserAction && legacySend) {
    window.ozzoSendUserAction = legacySend;
  }
  if (!window.ozzoSendUserAction && typeof window.ozzoSendUserAction === "function") {
    window.ozzoSendUserAction = window.ozzoSendUserAction;
  }
  const hasHelper = () =>
    typeof window.ozzoSendUserAction === "function" ||
    typeof window.ozzoSendUserAction === "function";
  // Use textContent instead of innerHTML to prevent XSS
  const bridgeStatus = hasHelper() ? "ready" : "missing";
  const bridgeClass = hasHelper() ? "ok" : "bad";
  statusEl.textContent = "";
  const bridgeLabel = document.createTextNode("Bridge: ");
  const bridgeSpan = document.createElement("span");
  bridgeSpan.className = bridgeClass;
  bridgeSpan.textContent = bridgeStatus;
  const platformInfo = document.createTextNode(
    " · iOS=" + (hasIOS() ? "yes" : "no") +
    " · Android=" + (hasAndroid() ? "yes" : "no")
  );
  statusEl.appendChild(bridgeLabel);
  statusEl.appendChild(bridgeSpan);
  statusEl.appendChild(platformInfo);

  window.addEventListener("ozzo:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Ozzo node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.ozzoSendUserAction === "function"
        ? window.ozzoSendUserAction
        : window.ozzoSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OZZO_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "ozzo", "canvas"));
  ...
  if (wss) {
    wss.on("connection", (ws) => {
      // Security: Enforce max connection limit
      if (sockets.size >= MAX_WEBSOCKET_CONNECTIONS) {
      ...
      // Critical: Clean up on error to prevent memory leaks
      ws.on("error", (err) => {
        opts.runtime.error(`WebSocket error: ${String(err)}`);
        clearInterval(pingInterval);
        sockets.delete(ws);
        ws.terminate();
      });
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Ozzo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
