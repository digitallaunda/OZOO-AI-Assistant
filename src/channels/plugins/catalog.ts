...
import { discoverOzzoPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { OzzoPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  ozzo?: OzzoPackageManifest;
  [LEGACY_MANIFEST_KEY]?: OzzoPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["OZZO_PLUGIN_CATALOG_PATHS", "OZZO_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<OzzoPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageOzzo?: OzzoPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageOzzo;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.ozzo ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageOzzo: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOzzoPlugins({ workspaceDir: options.workspaceDir });
