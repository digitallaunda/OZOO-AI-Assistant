...
function setSlackDmPolicy(cfg: OzzoConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Ozzo";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Ozzo`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: OzzoConfig, allowFrom: string[]): OzzoConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
