...
function setWhatsAppDmPolicy(cfg: OzzoConfig, dmPolicy: DmPolicy): OzzoConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OzzoConfig, allowFrom?: string[]): OzzoConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OzzoConfig, selfChatMode: boolean): OzzoConfig {
...
async function detectWhatsAppLinked(cfg: OzzoConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Ozzo can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Ozzo" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("ozzo channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
