...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OZZO_STATE_DIR: process.env.OZZO_STATE_DIR,
    OZZO_CONFIG_PATH: process.env.OZZO_CONFIG_PATH,
    OZZO_GATEWAY_PORT: process.env.OZZO_GATEWAY_PORT,
    OZZO_PROFILE: process.env.OZZO_PROFILE,
  };

  beforeEach(() => {
    process.env.OZZO_STATE_DIR = "/tmp/ozzo-cli-state";
    process.env.OZZO_CONFIG_PATH = "/tmp/ozzo-cli-state/ozzo.json";
    delete process.env.OZZO_GATEWAY_PORT;
    delete process.env.OZZO_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OZZO_STATE_DIR !== undefined)
      process.env.OZZO_STATE_DIR = originalEnv.OZZO_STATE_DIR;
    else delete process.env.OZZO_STATE_DIR;

    if (originalEnv.OZZO_CONFIG_PATH !== undefined)
      process.env.OZZO_CONFIG_PATH = originalEnv.OZZO_CONFIG_PATH;
    else delete process.env.OZZO_CONFIG_PATH;

    if (originalEnv.OZZO_GATEWAY_PORT !== undefined)
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OZZO_PROFILE: "dev",
        OZZO_STATE_DIR: "/tmp/ozzo-daemon-state",
        OZZO_CONFIG_PATH: "/tmp/ozzo-daemon-state/ozzo.json",
        OZZO_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/bot.ozoo.gateway.plist",
    });
