...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("ozzo node install"), formatCliCommand("ozzo node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("ozzo node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OZZO_LOG_PREFIX: baseEnv.OZZO_LOG_PREFIX ?? "node",
