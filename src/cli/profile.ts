...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.ozzo${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OZZO_PROFILE = profile;

  const stateDir = env.OZZO_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.OZZO_STATE_DIR?.trim()) env.OZZO_STATE_DIR = stateDir;

  if (!env.OZZO_CONFIG_PATH?.trim()) {
    env.OZZO_CONFIG_PATH = path.join(stateDir, "ozzo.json");
  }

  if (profile === "dev" && !env.OZZO_GATEWAY_PORT?.trim()) {
    env.OZZO_GATEWAY_PORT = "19001";
  }
}
