...
${formatHelpExamples([
  ['ozzo agent --to +15555550123 --message "status update"', "Start a new session."],
  ['ozzo agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'ozzo agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'ozzo agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['ozzo agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.ozoo.bot/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.ozoo.bot/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['ozzo agents set-identity --agent main --name "Ozoo" --emoji "🐙"', "Set name + emoji."],
  ["ozzo agents set-identity --agent main --avatar avatars/ozzo.png", "Set avatar path."],
  ["ozzo agents set-identity --workspace ~/ozzo --from-identity", "Load from IDENTITY.md."],
  [
    "ozzo agents set-identity --identity-file ~/ozzo/IDENTITY.md --agent main",
