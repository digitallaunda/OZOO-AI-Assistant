...
const SANDBOX_EXAMPLES = {
  main: [
    ["ozzo sandbox list", "List all sandbox containers."],
    ["ozzo sandbox list --browser", "List only browser containers."],
    ["ozzo sandbox recreate --all", "Recreate all containers."],
    ["ozzo sandbox recreate --session main", "Recreate a specific session."],
    ["ozzo sandbox recreate --agent mybot", "Recreate agent containers."],
    ["ozzo sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["ozzo sandbox list", "List all sandbox containers."],
    ["ozzo sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["ozzo sandbox recreate --all", "Recreate all containers."],
    ["ozzo sandbox recreate --session main", "Recreate a specific session."],
    ["ozzo sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["ozzo sandbox recreate --browser --all", "Recreate only browser containers."],
    ["ozzo sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["ozzo sandbox explain", "Show effective sandbox config."],
    ["ozzo sandbox explain --session agent:main:main", "Explain a specific session."],
    ["ozzo sandbox explain --agent work", "Explain an agent sandbox."],
    ["ozzo sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.ozoo.bot/cli/sandbox")}\n`,
    )
    .action(() => {
