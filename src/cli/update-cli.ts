...
import { resolveOzzoPackageRoot } from "../infra/ozzo-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "ozzo doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "Fresh code, same OZZO. Miss me?",
  "Back and better. Did you even notice I was gone?",
  "Update complete. I learned some new tricks while I was out.",
  "Upgraded! Now with 23% more sass.",
  "I've evolved. Try to keep up.",
  "New version, who dis? Oh right, still me but shinier.",
  "Patched, polished, and ready. Let's go.",
  "OZZO upgraded. New build, same assistant.",
  ...
  "Upgraded! Stuff got fixed. Blame the changelog if it breaks.",
  "Update complete. Please don't look at the commit history.",
  "Version bump! Same chaos energy, fewer crashes (probably).",
];

const MAX_LOG_CHARS = 8000;
const DEFAULT_PACKAGE_NAME = "ozzo";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "ozzo"]);
const CLI_NAME = resolveCliName();
const OZZO_REPO_URL = "https://github.com/ozzo/ozzo.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "ozzo");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("ozzo@")) return trimmed.slice("ozzo@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.OZZO_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OZZO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OZZO_GIT_DIR points at a non-git directory: ${params.dir}. Set OZZO_GIT_DIR to an empty folder or a ozzo checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OZZO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Ozzo update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Ozzo..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Ozzo install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("ozzo doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("ozzo gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g ozzo@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g ozzo@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OZZO_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OZZO_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("ozzo gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("ozzo doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("ozzo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("ozzo gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `ozzo update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OZZO_GIT_DIR points at a non-git directory: ${gitDir}. Set OZZO_GIT_DIR to an empty folder or a ozzo checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OZZO_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Ozzo to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["ozzo update", "Update a source checkout (git)"],
        ["ozzo update --channel beta", "Switch to beta channel (git + npm)"],
        ["ozzo update --channel dev", "Switch to dev channel (git + npm)"],
        ["ozzo update --tag beta", "One-off update to a dist-tag or version"],
        ["ozzo update --no-restart", "Update without restarting the service"],
        ["ozzo update --json", "Output result as JSON"],
        ["ozzo update --yes", "Non-interactive (accept downgrade prompts)"],
        ["ozzo update wizard", "Interactive update wizard"],
        ["ozzo --update", "Shorthand for ozzo update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run ozzo update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.ozoo.bot/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ozzo update status", "Show channel + version status."],
          ["ozzo update status --json", "JSON output."],
          ["ozzo update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.ozoo.bot/cli/update")}`,
    )
    .action(async (opts) => {
      try {
