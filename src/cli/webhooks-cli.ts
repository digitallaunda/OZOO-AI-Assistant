...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.ozoo.bot/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Ozzo hooks")
    ...
    .option("--hook-url <url>", "Ozzo hook URL")
    .option("--hook-token <token>", "Ozzo hook token")
