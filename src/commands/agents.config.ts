...
export function listAgentEntries(cfg: OzzoConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OzzoConfig, agentId: string) {
...
function resolveAgentModel(cfg: OzzoConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OzzoConfig): AgentSummary[] {
