...
function resolveDefaultAccountId(cfg: OzzoConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: OzzoConfig): boolean {
...
export function summarizeBindings(cfg: OzzoConfig, bindings: AgentBinding[]): string[] {
