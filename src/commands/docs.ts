...
const SEARCH_TOOL = "https://docs.ozoo.bot/mcp.SearchOzzo";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.ozoo.bot");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('ozzo docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.ozoo.bot/");
      runtime.log(`Search: ${formatCliCommand('ozzo docs "your query"')}`);
    }
    return;
  }
