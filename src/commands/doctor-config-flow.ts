...
function stripUnknownConfigKeys(config: OzzoConfig): {
  config: OzzoConfig;
  removed: string[];
} {
  const parsed = OzzoSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as OzzoConfig;
...
function noteOpencodeProviderOverrides(cfg: OzzoConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.OZZO_CONFIG_PATH?.trim() || env.OZZO_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "ozzo.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "ozzo.json");
  ...
  let cfg: OzzoConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as OzzoConfig;
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("ozzo doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "ozzo doctor --fix" to remove these keys.');
    }
  }
