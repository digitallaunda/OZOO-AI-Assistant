...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OZZO_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OZZO_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OZZO_GATEWAY_TOKEN");
    expect(message).not.toContain("OZZO_GATEWAY_PASSWORD");
  });
