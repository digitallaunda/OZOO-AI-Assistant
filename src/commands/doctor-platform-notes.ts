...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".ozzo", "disable-launchagent");
...
function hasConfigGatewayCreds(cfg: OzzoConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("OZZO_GATEWAY_TOKEN");
  const envPassword = await getenv("OZZO_GATEWAY_PASSWORD");
