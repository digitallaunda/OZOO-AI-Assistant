...
import type { OzzoConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OzzoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("ozzo security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("ozzo configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("ozzo config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("ozzo doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("ozzo config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
