...
beforeEach(() => {
  ...
  resolveOzzoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OZZO_STATE_DIR;
  originalUpdateInProgress = process.env.OZZO_UPDATE_IN_PROGRESS;
  process.env.OZZO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "ozzo-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OZZO_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OZZO_UPDATE_IN_PROGRESS;
  } else {
    process.env.OZZO_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOzzoPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOzzoPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/ozzo-root.js", () => ({
  resolveOzzoPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "ozzo-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OZZO_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OZZO_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/ozzo.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OZZO_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
