...
function resolveConfiguredModelRaw(cfg: OzzoConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: OzzoConfig): string[] {
...
export function applyPrimaryModel(cfg: OzzoConfig, model: string): OzzoConfig {
...
export function applyModelAllowlist(cfg: OzzoConfig, models: string[]): OzzoConfig {
  const defaults = cfg.agents?.defaults;
