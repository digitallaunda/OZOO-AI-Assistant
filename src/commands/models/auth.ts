...
import { readConfigFileSnapshot, type OzzoConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OzzoConfig, model: string): OzzoConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("ozzo plugins install")}\`.`,
    );
  }
