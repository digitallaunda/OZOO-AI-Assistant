...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("ozzo models auth setup-token")}\` or \`${formatCliCommand("ozzo configure")}\`.`
          : `Run \`${formatCliCommand("ozzo configure")}\` or set an API key env var.`;
