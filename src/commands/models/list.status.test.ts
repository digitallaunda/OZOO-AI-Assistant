...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOzzoAgentDir: vi.fn().mockReturnValue("/tmp/ozzo-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/ozzo-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOzzoAgentDir: mocks.resolveOzzoAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/ozzo-agent/auth-profiles.json");
