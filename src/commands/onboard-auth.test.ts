...
const requireAgentDir = () => {
  const agentDir = process.env.OZZO_AGENT_DIR;
  if (!agentDir) throw new Error("OZZO_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OZZO_STATE_DIR;
  const previousAgentDir = process.env.OZZO_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OZZO_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.OZZO_AGENT_DIR;
    } else {
      process.env.OZZO_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OZZO_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OZZO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-oauth-"));
    process.env.OZZO_STATE_DIR = tempStateDir;
    process.env.OZZO_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OZZO_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OZZO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-minimax-"));
    process.env.OZZO_STATE_DIR = tempStateDir;
    process.env.OZZO_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OZZO_AGENT_DIR;
