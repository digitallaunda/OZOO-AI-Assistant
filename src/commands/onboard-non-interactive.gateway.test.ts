...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.OZZO_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.OZZO_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.OZZO_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "ozzo");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OZZO_STATE_DIR = stateDir;
    process.env.OZZO_CONFIG_PATH = path.join(stateDir, "ozzo.json");
