...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Ozzo status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("ozzo security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("ozzo security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.ozoo.bot/faq");
  runtime.log("Troubleshooting: https://docs.ozoo.bot/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("ozzo status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("ozzo logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("ozzo status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("ozzo gateway probe")}`);
  }
}
