...
beforeAll(() => {
  previousProfile = process.env.OZZO_PROFILE;
  process.env.OZZO_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OZZO_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/ozzo-root.js", () => ({
  resolveOzzoPackageRoot: vi.fn().mockResolvedValue("/tmp/ozzo"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/ozzo/pnpm-lock.yaml",
      markerPath: "/tmp/ozzo/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/bot.ozoo.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/bot.ozoo.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Ozzo status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("ozzo status --all") ||
          l.includes("ozzo --profile isolated status --all") ||
          l.includes("ozzo status --all") ||
          l.includes("ozzo --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OZZO_GATEWAY_TOKEN = "abcd1234";
    try {
