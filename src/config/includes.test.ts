...
const ETC_OZZO_DIR = path.join(ROOT_DIR, "etc", "ozzo");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "ozzo.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOzzoPath(...parts: string[]) {
  return path.join(ETC_OZZO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOzzoPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "ozzo.json"))).toEqual({
      shared: true,
    });
  });
});
