...
import type { OzzoConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareOzzoVersions } from "./version.js";
...
function coerceConfig(value: unknown): OzzoConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OzzoConfig;
}
...
function stampConfigVersion(cfg: OzzoConfig): OzzoConfig {
...
function warnIfConfigFromFuture(cfg: OzzoConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareOzzoVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Ozzo (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OzzoConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OzzoConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OzzoConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OzzoConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OzzoConfig) {
    clearConfigCache();
...
// module scope. `OZZO_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OZZO_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OZZO_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): OzzoConfig {
...
export async function writeConfigFile(cfg: OzzoConfig): Promise<void> {
  clearConfigCache();
