  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.ozzo/logs/ozzo.log" },
        hooks: {
          path: "~/.ozzo/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".ozzo", "logs", "ozzo.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".ozzo", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".ozzo", "telegram.token"),
      );
