import type { OzzoConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: OzzoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OzzoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OzzoConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OzzoConfig): boolean {
...
function isIMessageConfigured(cfg: OzzoConfig): boolean {
...
function isWhatsAppConfigured(cfg: OzzoConfig): boolean {
...
function isGenericChannelConfigured(cfg: OzzoConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OzzoConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OzzoConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: OzzoConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OzzoConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: OzzoConfig, pluginId: string): OzzoConfig {
...
function enablePluginEntry(cfg: OzzoConfig, pluginId: string): OzzoConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
