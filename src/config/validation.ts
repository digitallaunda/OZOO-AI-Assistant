...
import type { OzzoConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OzzoConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OzzoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OzzoSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OzzoConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OzzoConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OzzoConfig)),
    ),
  };
}
