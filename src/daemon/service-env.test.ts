...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OZZO_GATEWAY_PORT).toBe("18789");
    expect(env.OZZO_GATEWAY_TOKEN).toBe("secret");
    expect(env.OZZO_SERVICE_MARKER).toBe("ozzo");
    expect(env.OZZO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OZZO_SERVICE_VERSION).toBe("string");
    expect(env.OZZO_SYSTEMD_UNIT).toBe("ozzo-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OZZO_LAUNCHD_LABEL).toBe("bot.ozoo.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OZZO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OZZO_SYSTEMD_UNIT).toBe("ozzo-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OZZO_LAUNCHD_LABEL).toBe("bot.ozoo.work");
    }
  });
});
