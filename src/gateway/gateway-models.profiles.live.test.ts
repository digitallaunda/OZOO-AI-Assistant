...
import { resolveOzzoAgentDir } from "../agents/agent-paths.js";
...
import { ensureOzzoModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { OzzoConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OZZO_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OZZO_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OZZO_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<OzzoConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.OZZO_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.OZZO_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveOzzoAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-live-state-"));
  process.env.OZZO_STATE_DIR = tempStateDir;
  ...
  process.env.OZZO_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.ozzo-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveOzzoAgentDir();
  const sanitizedCfg: OzzoConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Ozzo live tool probe (local, safe): " +
    ...
    process.env.OZZO_CONFIG_PATH = previous.configPath;
    process.env.OZZO_GATEWAY_TOKEN = previous.token;
    process.env.OZZO_SKIP_CHANNELS = previous.skipChannels;
    process.env.OZZO_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.OZZO_SKIP_CRON = previous.skipCron;
    process.env.OZZO_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.OZZO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OZZO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureOzzoModelsJson(cfg);
      ...
      const rawModels = process.env.OZZO_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.ozzo-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
