...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.OZZO_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "ozzo");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.ozzo-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".ozzo");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "ozzo.json");
      ...
      process.env.OZZO_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.OZZO_CONFIG_PATH = prev.configPath;
        process.env.OZZO_GATEWAY_TOKEN = prev.token;
        process.env.OZZO_SKIP_CHANNELS = prev.skipChannels;
        process.env.OZZO_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.OZZO_SKIP_CRON = prev.skipCron;
        process.env.OZZO_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.OZZO_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-wizard-home-"));
    process.env.HOME = tempHome;
    delete process.env.OZZO_STATE_DIR;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.OZZO_STATE_DIR = prev.stateDir;
      process.env.OZZO_CONFIG_PATH = prev.configPath;
      process.env.OZZO_GATEWAY_TOKEN = prev.token;
      process.env.OZZO_SKIP_CHANNELS = prev.skipChannels;
      process.env.OZZO_SKIP_GMAIL_WATCHER = prev.skipGmail;
