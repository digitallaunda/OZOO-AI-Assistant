...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Ozzo";
  if (/ozzo/i.test(trimmed)) return trimmed;
  return `${trimmed} (Ozzo)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.OZZO_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "ozzo");
  ...
  const binCli = path.join(cwd, "bin", "ozzo.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.OZZO_TAILNET_DNS?.trim();
