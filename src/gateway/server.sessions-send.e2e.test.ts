...
import { createOzzoTools } from "../agents/ozzo-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.OZZO_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.OZZO_GATEWAY_PORT = String(gatewayPort);
  process.env.OZZO_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OZZO_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createOzzoTools().find((candidate) => candidate.name === "sessions_send");
