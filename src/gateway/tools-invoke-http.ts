...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-ozzo-message-channel") ?? "");
  const accountId = getHeader(req, "x-ozzo-account-id")?.trim() || undefined;
  ...
  const allTools = createOzzoTools({
