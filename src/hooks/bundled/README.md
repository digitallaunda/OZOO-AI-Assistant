...
This directory contains hooks that ship with Ozzo. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.ozoo.bot/hooks/soul-evil
...
- **Managed hooks**: `~/.ozzo/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.ozzo/ozzo.json`:
...
2. Restart gateway: `pkill -9 -f 'ozzo.*gateway' && pnpm ozzo gateway`
3. Enable the hook: `ozzo hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.ozoo.bot/hooks
