...
homepage: https://docs.ozoo.bot/hooks#command-logger
metadata:
  {
    "ozzo":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.ozzo/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ozzo/logs/commands.log ~/.ozzo/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/ozzo`:
   ```
   /home/username/.ozzo/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.ozzo/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.ozzo/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.ozzo/logs/commands.log | jq .
```
