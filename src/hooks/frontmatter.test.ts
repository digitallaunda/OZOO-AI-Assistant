...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.ozzo.emoji).toBe("📝");
    expect(parsed.ozzo.events).toEqual(["command"]);
    expect(parsed.ozzo.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.ozzo.install[0].kind).toBe("bundled");
  });
...
describe("resolveOzzoMetadata", () => {
  it("extracts ozzo metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOzzoMetadata(frontmatter);
  ...
  it("returns undefined when ozzo key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        ozzo: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Ozzo" },
            { id: "npm", kind: "npm", package: "@ozzo/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@ozzo/hook");
  });
...
homepage: https://docs.ozoo.bot/hooks#session-memory
metadata:
  {
    "ozzo":
      {
...
`;
    ...
    const ozzo = resolveOzzoMetadata(frontmatter);
    expect(ozzo).toBeDefined();
    expect(ozzo?.emoji).toBe("💾");
    expect(ozzo?.events).toEqual(["command:new"]);
    expect(ozzo?.requires?.config).toEqual(["workspace.dir"]);
    expect(ozzo?.install?.[0].kind).toBe("bundled");
  });
