...
export function listIMessageAccountIds(cfg: OzzoConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: OzzoConfig): string {
...
function mergeIMessageAccountConfig(cfg: OzzoConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: OzzoConfig): ResolvedIMessageAccount[] {
