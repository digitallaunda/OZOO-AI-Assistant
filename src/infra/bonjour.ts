...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OZZO_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Ozzo";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Ozzo\)\s*$/i, "").trim() || normalized;
}
