...
const execSecuritySchema = z.enum(["deny", "allowlist", "full"]);
const execAskSchema = z.enum(["off", "on-miss", "always"]);

const execApprovalsDefaultsSchema = z.object({
...
const execAllowlistEntrySchema = z.object({
...
const execApprovalsAgentSchema = execApprovalsDefaultsSchema.extend({
  allowlist: z.array(execAllowlistEntrySchema).optional(),
});

const execApprovalsFileSchema = z.object({
  version: z.literal(1),
  socket: z
    .object({
...
const DEFAULT_SOCKET = "~/.ozzo/exec-approvals.sock";
const DEFAULT_FILE = "~/.ozzo/exec-approvals.json";
...
export function readExecApprovalsSnapshot(): ExecApprovalsSnapshot {
  ...
  try {
    ...
    const validated = execApprovalsFileSchema.safeParse(jsonData);
    if (validated.success) {
      parsed = validated.data;
    } else {
      // Log validation errors for debugging
      console.warn("Invalid exec-approvals file structure:", validated.error.issues);
      parsed = null;
    }
  } catch (err) {
    console.warn("Failed to parse exec-approvals file:", err);
    parsed = null;
  }
  const file =
    parsed?.version === 1
...
export function loadExecApprovals(): ExecApprovalsFile {
  ...
  try {
    ...
    console.warn("Failed to load exec-approvals:", err);
    return normalizeExecApprovals({ version: 1, agents: {} });
  }
}
