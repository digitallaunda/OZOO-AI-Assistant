...
function candidateBinDirs(opts: EnsureOzzoPathOpts): string[] {
  ...
  // Bundled macOS app: `ozzo` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingOzzo = path.join(execDir, "ozzo");
    if (isExecutable(siblingOzzo)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/ozzo` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "ozzo"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `ozzo` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOzzoCliOnPath(opts: EnsureOzzoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OZZO_PATH_BOOTSTRAPPED)) return;
  process.env.OZZO_PATH_BOOTSTRAPPED = "1";
