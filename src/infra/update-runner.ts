...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "ozzo doctor",
        managerScriptArgs(manager, "ozzo", ["doctor", "--non-interactive"]),
        gitRoot,
        { OZZO_UPDATE_IN_PROGRESS: "1" },
      ),
    );
