...
export const WIDE_AREA_DISCOVERY_DOMAIN = "ozzo.internal.";
export const WIDE_AREA_ZONE_FILENAME = "ozzo.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*ozzo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "ozzo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "ozzo");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "ozzo-gw");
  ...
  records.push(`_ozzo-gw._tcp IN PTR ${instanceLabel}._ozzo-gw._tcp`);
  records.push(`${instanceLabel}._ozzo-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._ozzo-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; ozzo-content-hash: ${contentHash}\n${contentBody}`;
}
