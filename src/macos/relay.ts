...
async function main() {
  ...
  const { ensureOzzoCliOnPath } = await import("../infra/path-env.js");
  ensureOzzoCliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[ozzo] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[ozzo] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
