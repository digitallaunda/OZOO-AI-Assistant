...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OZZO_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "ozzo-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OZZO_STATE_DIR = path.join(home, ".ozzo");
    ...
    await fs.mkdir(path.join(home, ".ozzo"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.ozzo${path.sep}media`);
