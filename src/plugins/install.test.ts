...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `ozzo-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.ozzo/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without ozzo.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@ozzo/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("ozzo.extensions");
  });
});
