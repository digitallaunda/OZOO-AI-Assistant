...
import { loadOzzoPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OZZO_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `ozzo-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "ozzo.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OZZO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOzzoPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OZZO_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOzzoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOzzoPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@ozzo/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        ozzo: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
