...
import { discoverOzzoPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OzzoPluginDefinition;
  register?: OzzoPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OzzoPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OzzoPluginDefinition;
...
export function loadOzzoPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOzzoPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OzzoPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OzzoPluginModule;
    } catch (err) {
