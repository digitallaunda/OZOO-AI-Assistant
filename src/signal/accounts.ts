...
export function listSignalAccountIds(cfg: OzzoConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: OzzoConfig): string {
...
function mergeSignalAccountConfig(cfg: OzzoConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: OzzoConfig): ResolvedSignalAccount[] {
