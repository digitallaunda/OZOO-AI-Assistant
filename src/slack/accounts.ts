...
export function listSlackAccountIds(cfg: OzzoConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: OzzoConfig): string {
...
function mergeSlackAccountConfig(cfg: OzzoConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: OzzoConfig): ResolvedSlackAccount[] {
