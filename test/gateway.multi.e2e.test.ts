...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `ozzo-e2e-${name}-`));
  const configDir = path.join(homeDir, ".ozzo");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          OZZO_CONFIG_PATH: configPath,
          OZZO_STATE_DIR: stateDir,
          OZZO_GATEWAY_TOKEN: "",
          OZZO_GATEWAY_PASSWORD: "",
          OZZO_SKIP_CHANNELS: "1",
          OZZO_SKIP_BROWSER_CONTROL_SERVER: "1",
          OZZO_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        OZZO_GATEWAY_TOKEN: inst.gatewayToken,
        OZZO_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OZZO_GATEWAY_PORT: String(gwA.port),
          OZZO_GATEWAY_TOKEN: gwA.gatewayToken,
          OZZO_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          OZZO_GATEWAY_PORT: String(gwB.port),
          OZZO_GATEWAY_TOKEN: gwB.gatewayToken,
          OZZO_GATEWAY_PASSWORD: "",
        }),
