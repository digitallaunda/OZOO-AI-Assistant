...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OZZO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.OZZO_STATE_DIR = path.join(base, ".ozzo");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "ozzo-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".ozzo", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
