...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as OzzoApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as OzzoApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as OzzoApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OzzoApp);
    await loadDevices(host as unknown as OzzoApp);
    await loadConfig(host as unknown as OzzoApp);
    await loadExecApprovals(host as unknown as OzzoApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as OzzoApp);
    await loadConfig(host as unknown as OzzoApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OzzoApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OzzoApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__OZZO_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OzzoApp, false),
    loadPresence(host as unknown as OzzoApp),
    loadSessions(host as unknown as OzzoApp),
    loadCronStatus(host as unknown as OzzoApp),
    loadDebug(host as unknown as OzzoApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OzzoApp, true),
    loadConfigSchema(host as unknown as OzzoApp),
    loadConfig(host as unknown as OzzoApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OzzoApp, false),
    loadCronStatus(host as unknown as OzzoApp),
    loadCronJobs(host as unknown as OzzoApp),
  ]);
}
