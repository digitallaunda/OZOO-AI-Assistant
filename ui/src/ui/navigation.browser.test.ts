...
import { OzzoApp } from "./app";
import "../styles.css";

const originalConnect = OzzoApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("ozzo-app") as OzzoApp;
...
beforeEach(() => {
  OzzoApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__OZZO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  OzzoApp.prototype.connect = originalConnect;
  window.__OZZO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/ozzo/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/ozzo");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/ozzo/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OZZO_CONTROL_UI_BASE_PATH__ = "/ozzo";
    const app = mountApp("/ozzo/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/ozzo");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/ozzo/sessions");
  });
